...
def convnet_init():
    ...
    """
    For training with only Block 1:
    conv_w5=tf.Variable(kaiming_normal([16, 16, 32, 10]))
    conv_b5 =tf.Variable(tf.zeros(10,))

    For training with Block 1 and 2:
    conv_w5=tf.Variable(kaiming_normal([8, 8, 32, 10]))
    conv_b5 =tf.Variable(tf.zeros(10,))

    For training with Block 1, 2 and 3:
...
def convnet_forward(x, params):
    ...
    x4_1_conv = tf.nn.conv2d(x3_5_pool, conv_w4,[1, 1, 1, 1], padding='SAME') + conv_b4
    x4_2_relu = tf.nn.relu(x4_1_conv)
    
    x5_1_conv = tf.nn.conv2d(x4_2_relu, conv_w5,[1, 1, 1, 1], padding='SAME') + conv_b5
...
def three_layer_convnet_test():
    ...
    with tf.device(device):
        ...
        #The initialisation for conv_w5 changes for block 5 depending on the filter size.
        #It is 16*16*32*10 while training with block 1
        #It is 8*8*32*10 while training with block 1 and 2
        #It is 4*4*64*10 while training with block 1, 2 and 3.
...
train_losses_size=len(train_losses)
test_accuracies_size=len(test_accuracies)

x1 = np.linspace(0, epochs, train_losses_size)
x2 = np.linspace(0, epochs, test_accuracies_size)

#Plotting the result of Training loss over the 50 epochs.
...
#Plotting the test accuracy over 50 epochs.
plt.figure(2,figsize=(7,5))
plt.plot(x2,test_accuracies)
plt.xlabel('num of Epochs')
...
with tf.Session() as sess:
    ...
    #evaluating the grid tensor to retrieve the kernel filters.
...
#                                              PART 3                                             #
    ...
    ############################################################################
      ...
      #run a convolution over the weights of the first layer to generate the response map of each filter (32 filters)
