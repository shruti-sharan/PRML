...
class Ada_Weak_Classifier(Weak_Classifier):
	...
	def calc_error(self, weights, labels):
		...
		minval=np.amin(self.activations)
		maxval=np.amax(self.activations)
		m=len(labels)
		threshold=np.linspace(minval,maxval,self.num_bins)
		error=[]
		polarity=np.ones(self.num_bins)
		#compute error
		error=np.matmul(((threshold[:,None]<self.activations).astype(int)!=(labels*(labels==1))[None,:]),weights)
		indexes=error>0.5             #check for reverse polarity
		error[indexes]=1-error[indexes]
		polarity[indexes]=-1
		error=np.asarray(error,dtype=np.float64)
		min_error=np.amin(error)
		min_error_index=np.argmin(error)
		self.threshold=threshold[min_error_index]
		self.polarity=polarity[min_error_index]
...
class Real_Weak_Classifier(Weak_Classifier):
	def __init__(self, id, plus_rects, minus_rects, num_bins):
		super().__init__(id, plus_rects, minus_rects, num_bins)
		self.thresholds = None #bin edges
		self.bin_pqs = None #pq values of each bin
		self.train_assignment = None #which image belongs to which bin
	...
	def calc_error(self, weights, labels):
		...
		error=0
		bin_pqs=[[],[]]
		self.thresholds=np.linspace(minval,maxval,self.num_bins)
		self.train_assignment=np.digitize(self.activations,self.thresholds,right=True)
		
		for bin_index in range(self.num_bins):
			weights_bin=weights[self.train_assignment==bin_index]
			label_bin=labels[self.train_assignment==bin_index]
			bin_pqs[0].append(np.sum(weights_bin[label_bin==1]))
			bin_pqs[1].append(np.sum(weights_bin[label_bin==-1]))
			
		self.bin_pqs=np.asarray(bin_pqs)
		self.bin_pqs[self.bin_pqs==0]=1e-7     #checking if bin is empty and assinging a very small value to avoid NAN error.
			
		error+=np.sum(np.sqrt(self.bin_pqs[0]*self.bin_pqs[1])) #compute error
		return error*2
