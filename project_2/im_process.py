...
def nms(xyxys, overlap_thresh):
	...
	if len(xyxys) == 0:
		return []
 	# if the bounding boxes integers, convert them to floats since we divide
	...
	# extract the coordinates and the scores of the patches
	x1 = xyxys[:,0]
	y1 = xyxys[:,1]
	x2 = xyxys[:,2]
	y2 = xyxys[:,3]
	...
	# compute the area of the patches and sort the scores
	area = (x2 - x1 + 1) * (y2 - y1 + 1)
	idxs = np.argsort(score)
	idxs = np.array_split(idxs,2)[1]
	# keep looping while some indexes still remain in the indexes list
	while len(idxs) > 0:
		# extract the last index in the indexes list and add the index value to the list of picked indexes
		last = len(idxs) - 1
		i = idxs[last]
		pick.append(i)
